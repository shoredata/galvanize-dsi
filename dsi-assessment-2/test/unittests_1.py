...
from src import assessment_1 as a
...
class TestAssessment2(unittest.TestCase):
    ...
    def test_pandas_max_rent(self):
        ...
        #df = a.pandas_max_rent(pd.read_csv('data/rent.csv'))
    ...
    def test_sql_highest_rent_increase(self):
        ...
        # results are inconsistent .. this is in psql:
        # bart=# SELECT neighborhood FROM rent WHERE city='San Francisco' ORDER BY med_2014 - med_2011 DESC LIMIT 5;
        #    neighborhood
        # -------------------
        #  Parkside
        #  Excelsior
        ...
        #  Outer Sunset
        # (5 rows)
        # answer = {'Parkside', 'Excelsior', 'Visitacion Valley', 'Bayview', 'Outer Sunset'}
        # self.assertEqual(set(x[0] for x in result), answer)

        # import psycopg2
        # conn_string = "host='localhost' dbname='bart'"
        # conn = psycopg2.connect(conn_string)
        # cur = conn.cursor()
        # cur.execute("SELECT neighborhood FROM rent WHERE city='San Francisco' ORDER BY med_2014 - med_2011 DESC LIMIT 5;")
        # cur.fetchall()
        # # Out[11]:
        # # [('Parkside',),
        # #  ('Excelsior',),
